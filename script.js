document.addEventListener('DOMContentLoaded', () => {
    ...
    async function simulateFileSync() {
        ...
        try {
            ...
            // Define the categories to scan with their respective prefixes
            ...
            const categories = Object.keys(categoryPrefixes);
            let cardCount = 0;
            
            // For each category, try to fetch card files with the correct naming convention
            for (const category of categories) {
                const prefix = categoryPrefixes[category];
                
                // Try different numbering patterns (001 to 099)
                for (let i = 1; i <= 99; i++) {
                    // Format number with leading zeros
                    const num = i.toString().padStart(3, '0');
                    
                    // Try the correct file naming pattern: [PREFIX]-XXX-Title.json
                    // We'll try a few common title patterns to increase chances of finding files
                    const possibleTitles = [
                        'Sample', 'Guide', 'Tactic', 'Strategy', 'Method', 'Technique', 'Approach', 'System'
                    ];
                    
                    // First try without title (simpler format)
                    const fileNames = [`[${prefix}]-${num}.json`];
                    
                    // Then try with common titles
                    possibleTitles.forEach(title => {
                        fileNames.push(`[${prefix}]-${num}-${title}.json`);
                    });
                    ...
                    for (const fileName of fileNames) {
                        try {
                            ...
                            if (response.ok) {
                                ...
                                // Found the file, no need to try other titles for this number
                                break;
                            }
                        } catch (e) {
